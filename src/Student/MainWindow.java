...
import solveQuiz.ReadQuiz;
import solveQuiz.ShowQuizMain;
...
//                fileChooser.setCurrentDirectory(new File(("C:\\Users\\Hamid\\Downloads\\BSSE-7A\\Data Visualization")));
                ...
                if (result == JFileChooser.APPROVE_OPTION) {
                    ...
                    try {
                        ...
                        if (!status.equalsIgnoreCase("ERROR")) {
                            boolean status2 = UploadFile.UploadFileDetailsInDB(Constants.ClassSession_Id, status, Constants.StudentName);
                            if (status2) {
    ...
    MainWindow(ConnectToServer connectToServerObj, ScreenSharing screenSharingObj) throws UnknownHostException {
        this();
...
//            System.out.println(mzessage);
            if (action.equals("_FILE")) {
                ...
                if (screenActor.equalsIgnoreCase("Teacher") && screenStatus.trim().equalsIgnoreCase("ON")) {
                    ...
                    connectToServerObj.sendMessage("!@#@!_SCREEN!@#@!" + "Student!@#@!PORT" + "!@#@!" + Constants.StudentMulticastPORT);
    ...
    MainWindow(boolean srartServer) throws InterruptedException, IOException {
        try {
            screenSharingObj = new ScreenSharing();
//            screenSharingObj.startSendingScreen();

            ConnectToServer connectToServerObj = new ConnectToServer();
            MainWindow mainWindow = new MainWindow(connectToServerObj, screenSharingObj);
...
//        ConnectToServer clientobj = new ConnectToServer("127.0.0.1", "Hamid");
